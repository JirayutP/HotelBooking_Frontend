  ...
  return (
    <div className="max-w-lg border border-gray-300 rounded-lg shadow-lg overflow-hidden bg-white flex flex-col sm:flex-row m-4 hover:shadow-xl transition-shadow duration-200">
      <div className="p-4 flex-grow overflow-hidden">
        ...
        {BookerName && (
          <p
            ...
            title={`Booked by: ${BookerName}`}
          >
            Booked by: <span className="font-medium">{BookerName}</span>
          </p>
        )}
        <p
          className="text-sm text-gray-600 mt-2 truncate"
          title={`Check-in date: ${formattedCheckIn}`}
        >
          Check-in date: <span className="font-medium">{formattedCheckIn}</span>
        </p>
        <p
          className="text-sm text-gray-600 truncate"
          title={`Check-out date: ${formattedCheckOut}`}
        >
          Check-out date:{" "}
      ...
      <div className="p-4 flex sm:flex-col justify-center items-center gap-2 border-t sm:border-t-0 sm:border-l border-gray-200">
        <button
          className="block w-full sm:w-auto px-4 py-2 text-sm font-medium text-white bg-sky-600 hover:bg-indigo-600 rounded-md shadow transition-all duration-200"
          onClick={() => {
